...
// 解析一个entry（共享key长度、非共享key长度、value长度）,并返回key_delta的首地址
 ...
 public:
  Iter(const Comparator* comparator, const char* data, uint32_t restarts,
      ...
      : comparator_(comparator),
        ...
        current_(restarts_),
        restart_index_(num_restarts_)                    // 指向最后
  {
  ...
  bool ParseNextKey() {
    ...
    if (p == nullptr || key_.size() < shared) {
      ...
      while (restart_index_ + 1 < num_restarts_ &&
             GetRestartPoint(restart_index_ + 1) < current_)  //如果下一个restart_index_对应的entry的offset小于current_,则向后寻找在current_之后的restart point
      {
        ++restart_index_;
      }
      return true;
